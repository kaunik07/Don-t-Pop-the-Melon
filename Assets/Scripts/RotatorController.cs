...
public class RotatorController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(player.getGameState() == DontPopTheMelon.game_states.game_in_progress && colliding && Input.GetKeyDown(KeyCode.E)){
            ePress = true;
        }
        //Register a q press if player is colliding with a rotator 
        if (player.getGameState() == DontPopTheMelon.game_states.game_in_progress && colliding && Input.GetKeyDown(KeyCode.Q)){
            qPress = true;
        }
    }
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.CompareTag("Player"))
        {
            if (player.getGameState() == DontPopTheMelon.game_states.game_in_progress)
            {
